...
选择 Surge 时，可将“Surge 自动测速节点”设为“仅 AnyTLS”。这会把 `surge_preferences.auto_test_protocols` 作为显式 Profile 意图保存，只过滤 Surge 产物中的自动测速组；手动选择仍保留所有兼容节点。选择“全部兼容节点”或读取没有该字段的旧 Profile 时，不应用协议过滤。
...
主备故障切换始终要求两个明确节点。主备检查和自动测速使用通用连通性地址，不能据此判断 ChatGPT 等服务是否接受该出口，也不会自动授权 AI 服务跨节点切换。新 Profile 保存服务偏好，编译时读取当前 Leo 和服务目录；旧策略快照保持原语义，直到明确预览并保存升级。
...
- **Surge 原生输入 → Surge**：保留 `[General]`、`[Host]`、`[Proxy]` 及其他非分流段，包括专用 DNS、DoH、测速地址、超时、IPv6 和节点原始参数。只替换 `[Rule]` 并增加所需策略组；保留原组及代理链引用。内部规范化的节点选择映射回原名，有歧义的名称冲突明确报错。移除上游 `#!MANAGED-CONFIG` 指令，避免更新回机场原始规则；通过 Subflow 订阅链接刷新。对局域网无认证访问、旧式 DoH 名称、`info` 日志、完整隧道范围及关闭 TLS 证书验证的节点只生成脱敏建议，不自动改写来源。`skip-cert-verify` 尤其不会被自动关闭，需由机场提供可验证证书后再人工调整。
