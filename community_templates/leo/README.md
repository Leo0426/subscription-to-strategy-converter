...
- 扫描完整配置：59 份。
- 原始规则：2103 条；当前模板收敛为 132 条，其中仅 8 条 `RULE-SET`。OpenAI 核心域名内联以保证 Surge 主链路；Telegram 使用一份同时覆盖域名与无域名 IP 流量的 classical 规则源。
...
- 2026-09-08 公开审计快照与模板 SHA 匹配：8 个来源全部可用，0 个格式无效，0 个完整重复组，0 个高重叠对。审计工具使用客户端等效 UA（`clash.meta/1.18.0 (subflow-rule-audit)`），避免把来源的 UA 白名单误判为不可用。
