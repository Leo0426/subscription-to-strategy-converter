...
- 2026-07-27 审计：508 个来源，470 个本轮可用，0 个格式无效；38 个观察项（24 个 HTTP 403、13 个连接失败、1 个镜像域名解析受限），均保留待跨轮确认。
- 结构评分公式 v2（含供应链与冷启动维度）：85.09/100（B）；与旧版 95.95（A）不可比。短板是供应链（全部来源不可固定版本，48 个经第三方代理中转）与冷启动成本（508 个来源超出 200 的数量预算，总下载量 15.0 MiB 贴近 16 MiB 预算）。
